...
TO DO:
    ...
    При выводе истории должны запрашиваться конкретные категории, записи по которым и будут выводиться. Но также оставить возможность выводить всю историю.
    ДОБАВИТЬ НЕОБЯЗАТЕЛЬНОЕ ОПИСАНИЕ ЗАПИСИ. П-ЛЬ МОЖЕТ ДАТЬ ОПИСАНИЕ, А МОЖЕТ НЕ ДАВАТЬ.
    
Баги:
    Критические:
        бот сыпется, если слишком большая длина одной категории. сделать обработку - DONE
        убрать глобальные переменные - DONE
    Остальные:
        запретить добавление дублирующей категории - DONE
        Проверять длинну категорий по отдельности, а не длинну всего сообщения - DONE
        обработать исключение для ввода слишком большого кол-ва дней, за которые должна выводиться история - DONE
        Если кто то добавит категорию с названием начинающимся с del_, то вместо записи, категория удалиться
    Полировка:
        заменить технические названия и сделать вывод описания в начале, сделать удаление сообщений с командами - DONE
        красиво расположить кнопки на инлайн клавиатуре - DONE
"""
